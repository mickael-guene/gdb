			  ...
			  CORE_ADDR func)
{
  ...
  unsigned int first_handler_instruction = read_memory_unsigned_integer (handler & ~1, 4, byte_order);

  /* we look for either arm or thumb2 code */
  /* this only work well for libc registered handler */
  if (first_handler_instruction == ARM_SET_R7_RT_SIGRETURN || first_handler_instruction == THUMB2_SET_R7_RT_SIGRETURN)
    arm_linux_sigtramp_cache (this_frame, this_cache, func,
