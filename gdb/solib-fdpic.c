...
/*
 TODO :
    ...
    - internal TODOS :
      ...
      - remove_solib_event_breakpoints not call due to bug in gdb => enter bug report ?
*/
...
displacement_from_map (struct elf32_fdpic_loadmap *map, CORE_ADDR addr)
{
  ...
  for (seg = 0; seg < map->nsegs; seg++) {
    if (map->segs[seg].p_vaddr <= addr && addr < map->segs[seg].p_vaddr + map->segs[seg].p_memsz)
...
decode_loadmap (gdb_byte *buf)
{
  ...
  res = xmalloc (sizeof(struct elf32_fdpic_loadmap) + input->nsegs * sizeof(struct elf32_fdpic_loadseg));
  if (res) {
    int i;
...
{
  gdb_byte buf[sizeof(struct elf32_fdpic_loadmap) + 2 * sizeof(struct elf32_fdpic_loadseg)];
  /* TODO : Here I take a shortcut since nsegs is always 2 .... unless ? */
...
fdpic_relocate_main_executable (void)
{
  ...
  /* if we have modification then relocate */
  if (changed)
...
lm_base (void)
{
  ...
  if (got_sym == 0) {
    if (solib_fdpic_debug)
      fprintf_unfiltered (gdb_stdlog, "lm_base: _GLOBAL_OFFSET_TABLE_ not found.\n");
    return 0;
  }
  addr = SYMBOL_VALUE_ADDRESS (got_sym) + 8;

  if (solib_fdpic_debug)
    fprintf_unfiltered (gdb_stdlog, "lm_base: _GLOBAL_OFFSET_TABLE_ + 8 = %s\n",hex_string_custom (addr, 8));
  ...
  if (solib_fdpic_debug)
    fprintf_unfiltered (gdb_stdlog, "lm_base: lm_base_cache = %s\n", hex_string_custom (info->lm_base_cache, 8));
...
enable_break2 (void)
{
  ...
  if (interp_sect) {
    ...
    if (interp_sect) {
      ...
      info->interp_text_sect_low += displacement_from_map (ldm, info->interp_text_sect_low);
    ...
    if (interp_sect) {
      ...
      info->interp_plt_sect_low += displacement_from_map (ldm, info->interp_plt_sect_low);
      info->interp_plt_sect_high = info->interp_plt_sect_low + bfd_section_size (tmp_bfd, interp_sect);
    }
    ...
    if (solib_fdpic_debug)
      fprintf_unfiltered (gdb_stdlog, "enable_break: _dl_debug_addr (prior to relocation) = %s\n", hex_string_custom (addr, 8));
    addr += displacement_from_map (ldm, addr);
    if (solib_fdpic_debug)
      fprintf_unfiltered (gdb_stdlog, "enable_break: _dl_debug_addr (after relocation) = %s\n", hex_string_custom (addr, 8));

    /* read structure */
    if (target_read_memory (addr, (gdb_byte *) &addr, sizeof(addr)) != 0) {
      warning (_("Unable to fetch contents of _dl_debug_addr "
                 "(at address %s) from dynamic linker"), hex_string_custom (addr, 8));
    }
    ...
    /* TODO : we disable remove of solib event bp since it may crash gdb when
     * enable_break2 is call in solib event context. Since bp memory is return
     * but read later in bpstat_stop_status() after handle_solib_event () call.
     * and if bp memory is reallocated and modify then we can have a crash */
...
fdpic_find_global_pointer (CORE_ADDR addr)
{
  ...
  while (so) {
    ...
    for (seg = 0; seg < map->nsegs; seg++) {
      if (map->segs[seg].addr <= addr && addr < map->segs[seg].addr + map->segs[seg].p_memsz)
  ...
  /* Didn't find it in any of the shared objects.  So assume it's in the main executable. */
  return main_got ();
}
...
fdpic_current_sos (void)
{
  ...
  while (lm_addr) {
    ...
    if (target_read_memory (lm_addr, (gdb_byte *) &lm_buf, sizeof (lm_buf)) != 0) {
      warning (_("fdpic_current_sos: Unable to read link map entry. Shared object chain may be incomplete."));
      break;
    }
    ...
    if ((CORE_ADDR) lm_buf.l_addr.got_value != mgot) {
      ...
      if (loadmap == NULL) {
        warning (_("fdpic_current_sos: Unable to fetch load map. Shared object chain may be incomplete."));
        break;
      }
      ...
      /* Fetch the name */
      target_read_string ((CORE_ADDR) lm_buf.l_name, &name_buf, SO_NAME_MAX_PATH_SIZE - 1, &errcode);
      if (errcode != 0) {
        warning (_("Can't read pathname for link map entry: %s."), safe_strerror (errcode));
      } else {
        if (solib_fdpic_debug)
                                 ...
                                 struct target_section *sec)
{
  ...
  for (seg = 0; seg < map->nsegs; seg++) {
    if (map->segs[seg].p_vaddr <= sec->addr && sec->addr < map->segs[seg].p_vaddr + map->segs[seg].p_memsz) {
...
_initialize_fdpic_solib (void)
{
  ...
  solib_fdpic_pspace_data = register_program_space_data_with_cleanup (fdpic_pspace_data_cleanup);
