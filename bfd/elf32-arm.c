...
elf32_arm_nabi_grok_prstatus (bfd *abfd, Elf_Internal_Note *note)
{
  ...
  switch (note->descsz)
    ...
    if (!_bfd_elfcore_make_pseudosection(abfd, ".pr_exec_fdpic_loadmap_addr", 4, note->descpos + offset + size))
        return FALSE;
    if (!_bfd_elfcore_make_pseudosection(abfd, ".pr_interp_fdpic_loadmap_addr", 4, note->descpos + offset + size + 4))
        return FALSE;
